...
/// <summary>
/// A single-use email-verification token, stored only as a hash with a short expiry. The raw token is
/// delivered out-of-band (the "confirm your email" link) and never persisted. Consuming it is one-way.
/// Mirrors <see cref="PasswordResetToken"/>. (docs/04-security.md)
/// </summary>
public sealed class EmailVerificationToken : Entity
{
    ...
    private EmailVerificationToken(Guid id, Guid userId, string tokenHash, DateTimeOffset expiresAtUtc)
        : base(id)
    {
        UserId = userId;
        TokenHash = tokenHash;
